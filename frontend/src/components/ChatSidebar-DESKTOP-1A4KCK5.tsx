...
const ChatSidebar = ({ sidebarOpen, setSidebarOpen, showAllUsers, setShowAllUsers,
    users, loggedInUser, chats, selectedUser, setSelectedUser, handleLogout,
    ...
    return (
        ...
        <aside className={`fixed z-20 sm:static top-0 h-screen w-80 bg-gray-900 border-r
         border-gray-700 transform ${sidebarOpen ? "translate-x-0" : "translate-x-full"
        ...
        >
            ...
            <div className="flex-1 overflow-hidden px-4 py-2">
                {
                    showAllUsers ? <div className="space-y-4 h-full">
                        ...
                        <div className="space-y-2 overflow-y-auto h-full pb-4">
                            {users
                                ?.filter((u) => u._id !== loggedInUser?._id && u.name.toLowerCase().includes(searchQuery.toLowerCase()))
                                .map((u) => {
                                    const isUserSelected = selectedUser === u._id;
                                    return (
                                        <button
                                            key={u._id}
                                            // onClick={() => setSelectedUser(u._id)} ${isUserSelected ? "bg-blue-600 border-blue-500" : "border
                                            className={"w-full text-left p-4 rounded-lg transition-colors  border-gray-700 hover:border-gray-600 hover:bg-gray-800"
                                            } onClick={() => { createChat(u); setSidebarOpen(false) }}
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className="relative">
                                                    <UserCircle className="w-6 h-6 text-gray-300" />
                                                    {
                                                        onlineUsers.includes(u._id) && (
                                                            <span className="absolute -top-0.5 -right-0.5 -right-0.5 w-3.5 h-3.5
                        ...
                        chats && chats.length > 0 ? <div className="space-y-2 overflow-y-auto h-full pb-4">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className="relative">
                                                ...
                                                flex items-center justify-center">
                                                    ...
                                                    {/* online user work */}
                                                    {
                                                        chat.user?._id && onlineUsers.includes(chat.user._id) && (
                                            ...
                                            <div className="flex-1 min-w-0 ">
                                                <div className="flex items-center justify-between mb-1">
                                                    <span className={`font semi-bold truncate ${isSelected ? "text-white" : "text-gray-200"
                                                        }`}>
                            ...
                            <p className="text-gray-400 font-medium"> No Conersation Yet</p>
            ...
            <div className="p-4 border-t border-gray-700 space-y-2">
                <Link href={'/profile'} className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-800 transition-colors">
                    <div className="p-1.5 bg-gray-700 rounded-lg "><UserCircle className="w-4 h-4 text-gray-300" />
                    </div>
